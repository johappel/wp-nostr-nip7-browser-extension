...
export async function cacheDmMessage(msg, scope) {
  try {
    ...
    // Deduplizierung: innerId ist der primäre Dedup-Key (stabil über wrapForRecipient + wrapForSelf)
    // Fallback auf giftWrapId/id für ältere Cache-Einträge
    const dedupInnerId = msg.innerId;
    if (dedupInnerId && cache.messages.some(m => m.innerId === dedupInnerId)) return msg;

    const dedupId = msg.giftWrapId || msg.id;
    if (dedupId && cache.messages.some(m => (m.giftWrapId || m.id) === dedupId)) return msg;
