...
export function createRumor(senderPubkey, recipientPubkey, content) {
  const rumor = {
    ...
    content: content,
    pubkey: senderPubkey,
    // Keep the rumor deniable (no signature), but include an event id for
    // clients that deserialize rumors via strict Event.fromJson parsers.
    sig: ''
  };
...
export async function cacheDmMessage(msg, scope, ownerPubkey) {
  ...
  try {
    ...
    if (dedupInnerId && convo.some(m => m.innerId === dedupInnerId)) return null;

    const dedupId = msg.giftWrapId || msg.id;
    if (dedupId && convo.some(m => (m.giftWrapId || m.id) === dedupId)) return null;
